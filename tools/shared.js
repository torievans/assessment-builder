...
function numberLineSVG(config) {
  ...
  const BAR_W      = Math.max(380, partitions * 30);
...
// ─── Number Representations renderers ────────────────────────────────────────
// Moved from nr.js — pure SVG renderers with no state dependencies.
// State, UI, and config functions remain in tools/nr.js.
...
function nrDarkenHex(hex, factor=0.6) {
...
function nrRenderFrames(cfg) {
  ...
  function frameGroup(n, ox, oy, placedSoFar) {
    ...
    for (let row=0; row<rows; row++) {
      for (let col=0; col<cols; col++) {
        if (placed<n) {
          const tp=placedSoFar+placed, useC2=fSplitOn&&tp>=fSplit, {fill,stroke}=counterCol(useC2?fc2:fc1);
          const cx=(col*CELL+CELL/2).toFixed(1), cy=(row*CELL+CELL/2).toFixed(1), R=CELL*0.38;
          s += `<circle cx="${cx}" cy="${cy}" r="${R}" fill="${fill}" stroke="${stroke}" stroke-width="2"/>`;
          placed++;
        }
      }
    }
  ...
  const CELL=60, cols=5, rows=fType==='5'?1:2, FW=cols*CELL, FH=rows*CELL, cap=cols*rows, FGAP=20;
  const numF=Math.ceil(N/cap), lCols=Math.min(numF,2), lRows=Math.ceil(numF/lCols);
  const W=lCols*FW+(lCols-1)*FGAP, H=lRows*FH+(lRows-1)*FGAP;
  let groups='', rem=N, totalPlaced=0;
  for (let fi=0; fi<numF; fi++) {
    const fc=fi%lCols, fr=Math.floor(fi/lCols), n=Math.min(rem,cap);
    const {group}=frameGroup(n, fc*(FW+FGAP), fr*(FH+FGAP), totalPlaced);
    groups+=group; totalPlaced+=n; rem-=n;
  }
...
function nrRenderMultilink(cfg) {
  ...
  for(let i=0;i<N;i++){
    ...
    let isLastInRow;
    if(!colFill){isLastInRow=col===rowSize-1||i===N-1;}
    else if(col===rowSize-1){isLastInRow=true;}
