...
// ── State ──────────────────────────────────────────────────────
let nlStyle='through',nlColour='grey',nlJumpOpen=false,nlJump2Open=false,nlAnswerCircle=false,nlRevealAnswer=true;
...
function initNLPanel(){
  ...
  document.querySelectorAll('.tog-btn[data-reveal]').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('.tog-btn[data-reveal]').forEach(x=>x.classList.remove('active'));b.classList.add('active');nlRevealAnswer=b.dataset.reveal==='yes';autoPreviewNL();}));
...
function setupJumpCalc(fId,tId,lId){
  ...
  // Track whether the user has explicitly cleared the label so we don't auto-refill it.
  let userClearedLabel=false;
  l.addEventListener('input',()=>{
    const fv=parseFloat(f.value),tv=parseFloat(t.value),lv=parseJL(l.value);
    if(l.value.trim()===''){userClearedLabel=true;}
    else{userClearedLabel=false;if(!isNaN(fv)&&!isNaN(lv))t.value=fv+lv;else if(!isNaN(tv)&&!isNaN(lv))f.value=tv-lv;}
    autoPreviewNL();
  });
  const upd=()=>{const fv=parseFloat(f.value),tv=parseFloat(t.value),lv=parseJL(l.value);if(!isNaN(fv)&&!isNaN(tv)&&!userClearedLabel)l.value=fmtJL(tv-fv);else if(!isNaN(fv)&&!isNaN(lv))t.value=fv+lv;else if(!isNaN(tv)&&!isNaN(lv))f.value=tv-lv;autoPreviewNL();};
...
function restoreNLConfig(cfg){
  ...
  nlRevealAnswer=cfg.revealAnswer!==false; // default true
  document.querySelectorAll('.tog-btn[data-reveal]').forEach(b=>b.classList.toggle('active',b.dataset.reveal===(nlRevealAnswer?'yes':'no')));
  // Colour swatch
...
function nlPanelHTML(){
  return `<div class="form-row">
              <div class="field field-grow"><label>Question text<button class="apply-all-btn" onclick="applyTextToAll('nl-text')" title="Apply this stem to every question in the nugget">Apply to all</button></label><textarea id="nl-text" rows="2" placeholder="e.g. What number is missing?" style="resize:vertical;font:inherit;width:100%;box-sizing:border-box;padding:6px 10px;border:1px solid var(--border,#D1D5D8);border-radius:8px;background:var(--input-bg,#fff);color:inherit;line-height:1.4"></textarea></div>
